...
func (p *Provider) CertificateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if algorithm == algorithmEc256 {
		...
		if err != nil {
			log.ErrorContext(ctx, "EC public key from PKCS11", "err", err)
			panic(err)
		}
	...
	if err != nil {
		...
		log.ErrorContext(ctx, "RSA public key from PKCS11", "err", err)
		return
	}
	log.DebugContext(ctx, "Cert Handler found", "cert", certificatePem)
	jData, err := json.Marshal(certificatePem)
	if err != nil {
		http.Error(w, "serialization error", http.StatusInternalServerError)
		log.ErrorContext(ctx, "json certificate Marshal", "err", err)
		return
	}
...
func (p *Provider) PublicKeyHandlerV2(w http.ResponseWriter, r *http.Request) {
	...
	if format == "jwk" {
		...
		if err != nil {
			http.Error(w, "configuration error", http.StatusInternalServerError)
			log.ErrorContext(ctx, "failed to parse JWK", "err", err)
			return
		}
		...
		if err != nil {
			http.Error(w, "configuration error", http.StatusInternalServerError)
			log.ErrorContext(ctx, "failed to marshal JWK", "err", err)
			return
		}
		w.Header().Set("Content-Type", "application/json")
	...
	if err != nil {
		http.Error(w, "configuration error", http.StatusInternalServerError)
		log.ErrorContext(ctx, "export RSA public key", "err", err)
		return
	}
