...
if pkcs11-tool --allow-sw --show-info --list-objects; then
...
# verify login
pkcs11-tool --show-info --list-objects ||
...
if [ -z "${KAS_PRIVATE_KEY}" ]; then
  if [ -f kas-private.pem ]; then
    ...
    pkcs11-tool --pin "${PKCS11_PIN}" "${MODULE_ARG}" --write-object kas-private.pem --type privkey --label "${PKCS11_LABEL_PUBKEY_RSA}"
    pkcs11-tool --pin "${PKCS11_PIN}" "${MODULE_ARG}" --write-object kas-cert.pem --type cert --label "${PKCS11_LABEL_PUBKEY_RSA}"
  else
  ...
  pkcs11-tool --pin "${PKCS11_PIN}" "${MODULE_ARG}" --write-object <(echo "${KAS_PRIVATE_KEY}") --type privkey --label "${PKCS11_LABEL_PUBKEY_RSA}"
  pkcs11-tool --pin "${PKCS11_PIN}" "${MODULE_ARG}" --write-object <(echo "${KAS_CERTIFICATE}") --type cert --label "${PKCS11_LABEL_PUBKEY_RSA}"
fi
...
if [ -z "${KAS_EC_SECP256R1_PRIVATE_KEY}" ]; then
  if [ -f kas-ec-private.pem ]; then
    ...
    pkcs11-tool --pin "${PKCS11_PIN}"  "${MODULE_ARG}" --write-object kas-ec-private.pem --type privkey --label "${PKCS11_LABEL_PUBKEY_EC}"
    # import EC cert to PKCS
    pkcs11-tool --pin "${PKCS11_PIN}"  "${MODULE_ARG}" --write-object kas-ec-cert.pem --type cert --label "${PKCS11_LABEL_PUBKEY_EC}"
  else
  ...
  pkcs11-tool --pin "${PKCS11_PIN}"  "${MODULE_ARG}" --write-object <(echo "$KAS_EC_SECP256R1_PRIVATE_KEY") --type privkey --label "${PKCS11_LABEL_PUBKEY_EC}"
  pkcs11-tool --pin "${PKCS11_PIN}"  "${MODULE_ARG}" --write-object <(echo "$KAS_EC_SECP256R1_CERTIFICATE") --type cert --label "${PKCS11_LABEL_PUBKEY_EC}"
fi
